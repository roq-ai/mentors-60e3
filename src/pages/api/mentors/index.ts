...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getMentors() {
    ...
    const response = await prisma.mentor
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'mentor'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createMentor() {
    ...
    if (body?.available_hours?.length > 0) {
      const create_available_hours = body.available_hours;
      body.available_hours = {
        create: create_available_hours,
      };
    } else {
      delete body.available_hours;
    }
