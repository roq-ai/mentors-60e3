...
function DashboardPage() {
  ...
  const appUserStat = useSWR<PaginatedInterface<AppUserInterface>>(
    () => `/app-users`,
    () => getAppUsers({ limit: 1, offset: 0 }),
  );

  const mentorStat = useSWR<PaginatedInterface<MentorInterface>>(
    () => `/mentors`,
    () => getMentors({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'App Users',
      result: appUserStat?.data?.totalCount,
      isLoading: appUserStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Mentors',
      result: mentorStat?.data?.totalCount,
      isLoading: mentorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Appointments',
