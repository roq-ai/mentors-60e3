...
import { getAvailableHoursById, updateAvailableHoursById } from 'apiSdk/available-hours';
...
function AvailableHoursEditPage() {
  ...
  const handleSubmit = async (values: AvailableHoursInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAvailableHoursById(id, values);
      mutate(updated);
      resetForm();
