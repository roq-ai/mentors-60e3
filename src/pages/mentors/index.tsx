...
import { getMentors, deleteMentorById } from 'apiSdk/mentors';
import { MentorInterface } from 'interfaces/mentor';

type ColumnType = ColumnDef<MentorInterface, unknown>;
...
export function MentorListPage(props: MentorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getMentors({
        relations: ['user', 'appointment.count', 'available_hours.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MentorInterface>>(
    () => `/mentors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MentorInterface) => {
    if (hasAccess('mentor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/mentors/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('available_hours', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'available_hours',
          header: 'Available Hours',
          accessorKey: 'available_hours',
          cell: ({ row: { original: record } }: any) => record?._count?.available_hours,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/mentors/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('mentor', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/mentors/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('mentor', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="mentor" />
        </Flex>

        {hasAccess('mentor', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/mentors/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
