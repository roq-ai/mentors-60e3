...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('mentor', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('app_user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'app_user',
          header: 'App User',
          accessorKey: 'app_user',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/app-users/view/${record.app_user?.id}`}>
              {record.app_user?.interests}
            </Link>
          ),
        }
      : null,
